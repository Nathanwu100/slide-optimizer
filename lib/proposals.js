      ...
      proposals: {
        ...
        items: {
          ...
          properties: {
            ...
            // Short on purpose: this is echoed once per rewritten line, so a
            // long explanation is paid for on every paragraph in the deck.
            explanation: { type: "string", minLength: 1, maxLength: 120 },
          },
        },
      },
    },
...
export function validateProposalResponse(snapshot, input) {
  ...
  for (const proposal of incoming.slice(0, MAX_PROPOSALS)) {
    ...
    const explanation = cleanString(proposal.explanation, 120).trim();
