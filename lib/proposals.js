...
const BODY_REWRITE_WORDS = 11;
const TITLE_REWRITE_WORDS = 9;
...
export function proposalTargetKey(slide, objectId, originalText) {
  return `${Number(slide)}:${String(objectId)}:${String(originalText)}`;
}

/* Dense paragraphs must never disappear silently between the model and the
 * writer. These thresholds do not rewrite anything by themselves; they only
 * require the model to return a validated proposal or make the request fail
 * closed instead of producing a misleading partial presentation. */
export function requiredRewriteTargets(snapshot) {
  const targets = [];
  const seen = new Set();
  for (const slide of snapshot?.slides || []) {
    for (const element of slide.elements || []) {
      const minimumWords = element.type === "title" ? TITLE_REWRITE_WORDS : BODY_REWRITE_WORDS;
      for (const paragraph of element.paragraphs || []) {
        if (paragraph.editable === false || wordCount(paragraph.text) < minimumWords) continue;
        const key = proposalTargetKey(slide.slide, element.objectId, paragraph.text);
...
export function missingRequiredRewriteTargets(snapshot, proposals) {
  const covered = new Set((proposals || []).map((proposal) => (
    proposalTargetKey(proposal.slide, proposal.objectId, proposal.originalText)
  )));
  return requiredRewriteTargets(snapshot).filter((target) => !covered.has(target.key));
}
...
export function validateProposalResponse(snapshot, input) {
  ...
  for (const proposal of incoming.slice(0, MAX_PROPOSALS)) {
    ...
    const key = proposalTargetKey(slide, objectId, originalText);
    if (seen.has(key)) continue;
